...
- 首页四入口已完成新一轮候选视觉实现：保留原有语义和单击链路，改用单一雾面入口面板与本机 CSS 黄昏湖景，不引入网络图片或模型能力。
...
- 首页主问题为 `你这会儿，想从哪儿说起？`，四个核心状态继续一眼可见并可单击进入。
- 四项属于同一个入口面板，使用细分隔线和生活物件符号；不再使用序号、重复箭头、四张独立色卡或逐项解释。
- 场景使用本机 CSS 构成的黄昏湖面、远山、薄雾、芦苇和黄色空椅，不加载远程图片、SVG、Canvas 或自定义渲染循环。
- 持续动画只用于雾、水纹、倒影、芦苇和远景的低速变化，并必须服从 `prefers-reduced-motion`。
- 拍照与最近记录仍是低权重本机工具，不与四个状态入口争夺首页主位。
...
| A-UI | 四入口雾面场景视觉、入口去模板化、手机端可达性复核 | 候选完成 |
...
- 首页四个情绪入口的语义与单击链路保持不变；序号、重复箭头和逐项解释已移除，照片仍是其后的辅助工具行。
- 本机照片可进入时间线缩略图和详情页；单条删除与全部清空级联删除附件。
- Service Worker 只缓存应用壳和构建资源，明确排除 Blob/Data 与跨域请求。
- 42 项自动化测试通过；390×844 首页、分支与照片空态已完成浏览器复核，360px 与 200% 字号无页面级横向溢出，触控区不小于 44px。
