...
[![CI](https://github.com/oukeming64-tech/different-diary/actions/workflows/ci.yml/badge.svg)](https://github.com/oukeming64-tech/different-diary/actions/workflows/ci.yml)
[![GitHub Pages](https://github.com/oukeming64-tech/different-diary/actions/workflows/pages.yml/badge.svg)](https://github.com/oukeming64-tech/different-diary/actions/workflows/pages.yml)
[![Release](https://img.shields.io/github/v/release/oukeming64-tech/different-diary)](https://github.com/oukeming64-tech/different-diary/releases)
[![License: MIT](https://img.shields.io/badge/License-MIT-6b5d53.svg)](LICENSE)

[直接在线体验](https://oukeming64-tech.github.io/different-diary/) · [下载单文件版](https://oukeming64-tech.github.io/different-diary/download.html) · [查看 v0.1.0](https://github.com/oukeming64-tech/different-diary/releases/tag/v0.1.0) · [参与贡献](CONTRIBUTING.md)
...
- 按时间回看记录，单条删除、彻底清空，或保存完整备份并在本机恢复。
...
| 从现在的状态开始 | 先回应，再决定是否记录 | 最近发生的事只留在本机 |
| --- | --- | --- |
| <img src="public/showcase/home.png" alt="不一样的日记首页四个状态入口" width="260" /> | <img src="public/showcase/response.png" alt="想吃点东西后的非审判回应" width="260" /> | <img src="public/showcase/timeline.png" alt="保存在本机的最近记录" width="260" /> |
...
[打开下载与说明页面](https://oukeming64-tech.github.io/different-diary/download.html)，再点击页面里的“下载 HTML 文件”。
...
| 构建 | Vite 8；开发、测试和 GitHub Pages 使用同一条静态应用链路 |
| 本机数据 | IndexedDB + Dexie，包含原地版本迁移 |
| PWA | Web App Manifest + 同源 Service Worker |
| 测试 | Node Test Runner + `tsx` + `fake-indexeddb`，另做手机尺寸浏览器验收 |
| 可选 AI | OpenRouter 文字接口；Key 只留在当前页面内存 |
| 发布 | GitHub Actions → GitHub Pages，同时生成单文件 HTML |
...
页面入口在 [`app/page.tsx`](app/page.tsx)，实际职责按控制器、产品文案、首页、对话页和本机记录页拆在 [`app/diary/`](app/diary/)；本机文字与数据库在 [`lib/stage1/`](lib/stage1/)，照片、运动、可选 AI 和今日海报依次在 [`lib/stage2/`](lib/stage2/)、[`lib/stage3/`](lib/stage3/)、[`lib/stage4/`](lib/stage4/) 和 [`lib/poster/`](lib/poster/)；回归测试在 [`tests/`](tests/)，产品、技术和架构说明在 [`docs/`](docs/)。浏览器入口位于 [`github-pages/`](github-pages/)，构建配置集中在 [`vite.config.ts`](vite.config.ts)。
...
在自己的 GitHub 仓库中启用 Actions，并在 **Settings → Pages** 里选择 **GitHub Actions**，之后把通过检查的代码合并到 `main`，现有工作流就会自动发布网页和单文件下载版。如果更改了仓库名，需要同时修改 [`package.json`](package.json) 的 `build:pages --base`、[`scripts/build-standalone-html.mjs`](scripts/build-standalone-html.mjs) 中的 `basePath`，以及 README、下载页和分享图里的公开链接。
...
2026-09-05 主线更新：记录、照片和运动详情的完整 JSON v4 备份与恢复，入口在“翻翻最近 → 数据”。支持重复识别、冲突保留和事务回滚；不增加账户或后端。一次支持 128 MB 内文件，旧版照片索引不具备完整恢复能力。详见 [备份说明](docs/10-本机完整备份.md)。网页版与单文件下载版由同一次 Pages 构建发布。


截至 2026-07-11，手机端核心陪伴、本机文字与照片记录、本机运动记录、时间线与数据管理、PWA、可选文字 AI、新手引导和今日海报均已通过用户体验验收。图片理解、运动消耗估算、Key 的本机加密长期保存、独立 ZIP 容器与本机图表仍是后续计划（照片备份已通过单文件 JSON 实现），不属于当前版本。
